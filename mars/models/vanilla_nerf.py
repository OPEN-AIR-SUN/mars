...
from mars.fields.vanilla_nerf_field import NeRFField
...
class NeRFModel(Model):
    ...
    def populate_modules(self):
        ...
        if self.obj_feat_dim > 0:
            obj_position_encoding = NeRFEncoding(
                in_dim=3, num_frequencies=10, min_freq_exp=0.0, max_freq_exp=8.0, include_input=True
            )
        else:
            obj_position_encoding = None

        self.field_coarse = NeRFField(
            position_encoding=position_encoding,
            direction_encoding=direction_encoding,
            obj_position_encoding=obj_position_encoding,
    ...
    def num_sample_points(self) -> int:
        return self.config.num_coarse_samples + self.config.num_importance_samples + 1

    def get_param_groups(self) -> List[Parameter]:
        param_groups = []
        if self.field_coarse is None or self.field_fine is None:
            raise ValueError("populate_fields() must be called before get_param_groups")
        param_groups += list(self.field_coarse.parameters()) + list(self.field_fine.parameters())
        if self.temporal_distortion is not None:
            param_groups += list(self.temporal_distortion.parameters())
        return param_groups
    ...
    def inference_without_render(self, ray_bundle: RayBundle):
        ...
        return {
            ...
            "field_outputs": field_outputs_fine,
        }
